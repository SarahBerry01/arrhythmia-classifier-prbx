...
def produce_files(major_classes, removed_classes, file_prefix, segments, annotations):
    """ write balanced and unbalanced train file plus an unbalanced test file

    Args:
        major_classes: The class being classified (can be more than 1)
        removed_classes: class to remove (once classified)
        file_prefix: class annotation
        segments: record segments
        annotations: corresponding annotations to segments
    """
...
def save_patient_samples(sample_id, segments, annotations, major, removed):
    """Save record for individuals

    Args:
        sample_id : patient number
        segments : patient segments
        annotations : corresponding annotations
        major : class to classify
        removed : class to remove (once classified)
    """
...
def main():
    ...
    produce_files(major_classes, removed_classes,
                  "phase_1", all_segments, all_annotations)
